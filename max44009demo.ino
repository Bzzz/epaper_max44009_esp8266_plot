...
Max44009 myLux(0x4A);  // 0x4a; scl = D1, sda = D6
...
void loop() {
 ...
 drawHeight = map(lux, 0, mapmax, 0, disph+1 );
...
void drawAxises()  //separate to keep stuff neater. This controls ONLY drawing background!
{
  ...
  display.print(lux);   //unscaled raw value
  display.setCursor(320, 40);
  display.print("lux");


  display.drawLine(0, 0, 0, disph, GxEPD_BLACK);
  display.drawLine(dispw, 0, dispw, disph, GxEPD_BLACK);
  
  display.drawLine(0, 0, dispw, 0, GxEPD_BLACK);
  display.drawLine(0, disph, dispw, disph, GxEPD_BLACK);
  ...
  display.setCursor(0.25*dispstepw, disph+1-count + 0.25*dispsteph);
  display.print(map(count, 0, disph+1, 0, mapmax ));
  }
