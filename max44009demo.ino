...
#include <Fonts/FreeSansBold22pt7b.h>
...
void loop() {
  lux = myLux.getLux();   err = myLux.getError();   if (err != 0)   {
    Serial.print("Error:\t");
    Serial.println(err);
  }   else   {
    Serial.print("lux:\t");
  ...
  drawHeight = map(lux, 0, mapmax, 0, disph + 1 );   sensorArray[0] = drawHeight;

  for (count = 1; count <= dispw + 1; count++ )   {
    if (filled == 'D' || filled == 'd')     {
      if (drawDirection == 'L' || drawDirection == 'l')       {
        display.drawPixel(count, disph + 1 - sensorArray[count - 1], drawcolor);
      }       else //else, draw dots from right to left
      {
        display.drawPixel(dispw + 1 - count, disph + 1 - sensorArray[count - 1], drawcolor);
      }
    }     else     {
      if (drawDirection == 'L' || drawDirection == 'l')       {
        if (slope == 'S' || slope == 'S')         {
          display.drawLine(count, dispw + 1, count, disph + 1 - sensorArray[count - 1], drawcolor);
        }         else         {
          display.drawLine(count, dispw + 1, count, disph + 1 - sensorArray[count - 1], GxEPD_BLACK);

        }
      }       else       {
        if (slope == 'S' || slope == 'S')         {
          display.drawLine(dispw + 1 - count, disph, dispw + 1 - count, disph + 1 - sensorArray[count - 1], drawcolor);
        }         else         {
          display.drawLine(dispw + 1 - count, disph, dispw + 1 - count, disph + 1 - sensorArray[count - 1], GxEPD_BLACK);
        }
      }
    }
  }
  ...
  drawAxises();   rcount++;   if (rcount >= readings) {
...
{
  display.setCursor(320, 20);   display.setFont(&FreeSansBold9pt8b);   display.setTextColor(GxEPD_BLACK);   display.print(lux);   //unscaled raw value   display.setCursor(320, 40);   display.print("lux");
  ...
  for (count = dispsteph; count < disph + 1; count += dispsteph)   {
    ...
    display.setCursor(0.25 * dispstepw, disph + 1 - count + 0.25 * dispsteph);     display.print(map(count, 0, disph + 1, 0, mapmax ));
  }
